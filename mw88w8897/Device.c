...
#ifdef ALLOC_PRAGMA
#pragma alloc_text (PAGE, MWIFIEXCreateDevice)
#endif
...
--*/
{
	DbgPrint("[MWIFIEX] MWIFIEXCreateDevice\n");
	NTSTATUS                            status;
	WDFDEVICE                           wdfDevice;
...
--*/
{
	DbgPrint("[MWIFIEX] ControllerCreateWdfDeviceWithNameAndSymLink\n");
	NTSTATUS status;
...
{
	DbgPrint("[MWIFIEX] ControllerWdfEvtDevicePrepareHardware\n");
	FuncEntry(TRACE_DEVICE);
...
{
	DbgPrint("[MWIFIEX] ControllerWdfEvtDeviceD0Entry\n");
...
{
	DbgPrint("[MWIFIEX] ControllerWdfEvtDeviceD0EntryPostInterruptsEnabled\n");
	FuncEntry(TRACE_DEVICE);
	UNREFERENCED_PARAMETER(WdfDevice);
...
{
	DbgPrint("[MWIFIEX] ControllerWdfEvtDeviceD0ExitPreInterruptsDisabled\n");
	FuncEntry(TRACE_DEVICE);
	UNREFERENCED_PARAMETER(WdfDevice);
...
{
	DbgPrint("[MWIFIEX] ControllerWdfEvtDeviceD0Exit\n");
	FuncEntry(TRACE_DEVICE);
...
{
	DbgPrint("[MWIFIEX] ControllerWdfEvtDeviceReleaseHardware\n");
	FuncEntry(TRACE_DEVICE);
	UNREFERENCED_PARAMETER(WdfDevice);
	UNREFERENCED_PARAMETER(WdfResourcesTranslated);
	FuncExit(TRACE_DEVICE, 0);
...
{
	DbgPrint("[MWIFIEX] ControllerWdfEvtCleanupCallback\n");
	FuncEntry(TRACE_DEVICE);
...
{
	DbgPrint("[MWIFIEX] ControllerEvtIoDeviceControl\n");
	DbgPrint("[MWIFIEX] Handling I/O control code 0x%x\n", IoControlCode);
	BOOLEAN handled;
    NTSTATUS status = STATUS_SUCCESS;
	...
	DbgPrint("[MWIFIEX] Unexpected I/O control code 0x%x\n", IoControlCode);
...
{
	DbgPrint("[MWIFIEX] ControllerEvtUdecxWdfDeviceQueryUsbCapability\n");
	...
	// Because we don't know what caps we're looking for
	return STATUS_SUCCESS;
}